...
namespace espressopp {
  ...
  void FixedPairList::beforeSendParticles(ParticleList& pl, OutBuffer& buf) {
      //std::cout << "FixedPairList::beforeSendParticles CALLED! " << std::endl;
  ...
  void FixedPairList::afterRecvParticles(ParticleList &pl, InBuffer& buf) {
    //std::cout << "FixedPairList::afterRecvParticles CALLED! " << std::endl;
    ...
    if (i != size) {
      ...
      throw(std::runtime_error("ATTETNTION:  recv particles might have read garbage"));
    }
  ...
  void FixedPairList::onParticlesChanged() {
    ...
    for (GlobalPairs::const_iterator it = globalPairs.begin(); it != globalPairs.end(); ++it) {
      ...
      if (p2 == NULL) {
          std::stringstream msg;
          msg << "onParticlesChanged error. Fixed Pair List particle p2 " << it->second << " (in pair with p1 " << it->first << ") does not exists here";
          //std::runtime_error(err.str());
          err.setException( msg.str() );
      }
      this->add(p1, p2);
