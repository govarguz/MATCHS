...
/*-------------------------------------------------------------*/
  ...
  {
    real time = timeRebuild.getElapsedTime();
    ...
    for (CellListAllPairsIterator it(cl); it.isValid(); ++it) {
      //int previously_REMOVETHIS = vlPairs.size();
      checkPair(*it->first, *it->second);
      pairs_checked++;
      LOG4ESPP_DEBUG(theLogger, "checking particles " << it->first->id() << " and " << it->second->id());

//      int RANK_INTEREST = 0;
//      if(getSystem()->comm->rank() == RANK_INTEREST){
//          std::cout << "VERLET LIST (" << vlPairs.size() << "): checking particles " << it->first->id() << "(" 
//              << it->first->position() << ") and " << it->second->id() << " (" << it->second->position() << ") " 
//              << ((vlPairs.size() > previously_REMOVETHIS) ? "ADDED" : "NOT AD" ) << std::endl;
//      }
      
    }

    builds++;
    builds_time += timeRebuild.getElapsedTime() - time;
  ...
  ****************************************************/
  ...
  void VerletList::registerPython() {
    ...
    class_<VerletList, shared_ptr<VerletList> >
      ...
      .add_property("buildsTime", &VerletList::getBuildsTime, &VerletList::setBuildsTime)
      .def("buildsTimeFunc", &VerletList::getBuildsTime)
