...
def halfDecomp(adrCenter1D,rc_skin,eh_size,halfCores1D,cellsX,ratioMS,idealGas, halfCellInt = 1):
    ...
    for i in xrange(halfCores1D):
        ...
        if idealGas:
            if i==0:
                # not a good name, should be cellsPerProcessor or something
                ...
                [cellSizes.append(halfCellInt * round((eh_size)/rc_skin/(halfCores1D-usedCores)-0.5)) for i in xrange(usedCores,halfCores1D)]
                deltaCells=halfCellInt * round((eh_size)/rc_skin-0.5)-halfCellInt * round((eh_size)/rc_skin/(halfCores1D-usedCores)-0.5)*(halfCores1D-usedCores)
            ...
            if totNodesCG/2.>=2.:
                [cellSizes.append(halfCellInt * round((adrCenter1D-eh_size)/rc_skin/totNodesCG/2.-0.5)) for j in xrange(int(totNodesCG/2.))]
            else:
                cellSizes.append(halfCellInt * round((adrCenter1D-eh_size)/rc_skin-0.5))
            if totNodesEH/2.>=2.:
                [cellSizes.append(halfCellInt * round((eh_size)/rc_skin/(totNodesEH/2.)-0.5)) for i in xrange(int(totNodesEH/2.))]
            else:
