...
*  `nodeGrid(box_size,rc,skin,n,eh_size=0,ratioMS=0,idealGas=0,slabMSDims=[0,0,0])`:
    ...
    `idealGas` - this is a Flag for treating the low-resolution region as an Ideal Gas (no interactions included,
		 thus less load)
    `slabMSDims` _ 3D vector cointainig flags describing the type of axis, if heterogeneous value is 1, else 0
...
def neiListAdress(node_grid, cell_grid,rc,skin,eh_size,adrCenter,ratioMS,idealGasFlag=True,sphereAdr=False,slabMSDims=[1,0,0], halfCellInt = 1):
    ...
    cellsX=halfCellInt * round(cursor[0] / rc_skin-0.5)#round(cg_sizeL/rc_skin-0.5)+round((cursor[0]-cg_sizeR)/ rc_skin-0.5)+round((cg_sizeR-cg_sizeL)/rc_skin-0.5) # old implementation rounded # Cells too much!
