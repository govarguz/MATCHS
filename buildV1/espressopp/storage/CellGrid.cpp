...
longint CellGrid::mapPositionToCellWithGhosts(const Real3D& pos) const {
  int cpos[3];

  for(int i = 0; i < 3; ++i) {
    real lpos = pos[i] - myLeft[i] + frame * cellSize[i];
    if (lpos <= 0) cpos[i] = 0;
    else cpos[i] = static_cast<int>(lpos*invCellSize[i]);
  }
  return mapPositionToIndex(cpos);
}
