...
namespace espressopp {
  ...
  namespace storage {
    ...
    longint Storage::getNRealBoundaryParticles() const {
      longint cnt = 0;
      for (CellList::const_iterator it = realBoundaryCells.begin(), end = realBoundaryCells.end(); it != end; ++it) {
        longint size = (*it)->particles.size();
        if (size) {
          LOG4ESPP_TRACE(logger, "cell " << ((*it) - getFirstCell()) << " size " << size);
        }
    ...
    void Storage::resizeCells(longint nCells) {
      ...
      for(longint i = 0; i < cells.size(); i++)
          cells[i].id = i;
#endif
    }
...
//                                              std::endl;
    ...
    void Storage::removeAllParticlesNoSignal(){
      localParticles.clear();
      for (CellList::iterator it = localCells.begin(), end = localCells.end(); it != end; ++it) {
        (*it)->particles.clear();
      }
    ...
    void Storage::copyParticles(Storage &old)
    {

      localParticles.clear();

//      for(LocalCellList::Iterator it(cells), it_old(old.cells); it.isValid(); ++it, ++it_old)
//      {
//        (*it).particles.swap((*it_old).particles);
//        updateLocalParticles( (*it).particles );
//      }
      // first go through ghost cells, then through real cells
      // BEWARE, there might be both real and ghost particles with the same id on the same node.
      // this way, the real particle will be stored in localParticles
      // since updateLocalParticles is called later for the real cells and thus the reall particle would
      // overwrite the ghost particle
      for(CellList::Iterator it(ghostCells), it_old(old.ghostCells); it.isValid(); ++it, ++it_old)
      {
        (*it)->particles.swap((*it_old)->particles);
        updateLocalParticles( (*it)->particles );
      }

      for(CellList::Iterator it(realCells), it_old(old.realCells); it.isValid(); ++it, ++it_old)
      {
        (*it)->particles.swap((*it_old)->particles);
        updateLocalParticles( (*it)->particles );
      }
      //cells.swap(old.cells);
    }
...
//
//            std::cout << std::setprecision(4) << std::setw(5) << std::fixed << "  PACK POSITIONS ETC X      "
//                      << ": PARTICLE " << src->id() << " (" << src->position() << std::setprecision(2)
//                      << "), MPI " << getSystem()->comm->rank() << ", position shifted by " << shift[0] << "," << shift[1] << "," << shift[2] << std::endl;
...
//            std::cout << std::setprecision(4) << std::setw(5) << std::fixed << "UNPACK POSITIONS ETC 1      "
//                      << ": PARTICLE " << dst->id() << " (" << dst->position() << std::setprecision(2)
//                      << "), MPI " << getSystem()->comm->rank() << "extradata: " << extradata << std::endl;

        buf.read(*dst, extradata);

//            std::cout << std::setprecision(4) << std::setw(5) << std::fixed << "UNPACK POSITIONS ETC 2      "
//                      << ": PARTICLE " << dst->id() << " (" << dst->position() << std::setprecision(2)
//                      << "), MPI " << getSystem()->comm->rank() << std::endl;
...
#ifdef EXTRA_CHECKS
      if(reals.size() != ghosts.size())
        throw std::out_of_range("reals.size() != ghosts.size()");
#endif

      //std::cout << "SIZE " << reals.size() << ", " << ghosts.size() << std::endl;
...
#ifdef EXTRA_CHECKS
          if(dst->id() != src->id())
            throw std::runtime_error("real and ghost particle id does not match " + std::to_string(src->id()) + ", " +  std::to_string(dst->id()));
#endif
