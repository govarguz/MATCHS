...
*/
/* NOTE to E++ Developers: The initial CellGrid Structure within E++
   is backward compatible with HeSpaDDA, since the CellGrid implementation
  strictly fulfills the Linked-Cell-List algorithm. On top of that a feasible
  TODO is the implementation here of the half-cells, third cells and/or
   1/N cells algorithm. */
...
namespace espressopp {
  ...
  public:
    ...
    /// map coordinate to cell (ghost or non-ghost). This is used for DomainDecompositionDuplicate
    longint mapPositionToCellWithGhosts(const Real3D& pos) const;
    ...
    /// check whether a cell is in the inner grid and so far from boundary, that 
    /// it is not involved in communication
    bool isInnerNoCommCell(int m, int n, int o) const {
      return (m >= getInnerNoCommCellsBegin(0) && m < getInnerNoCommCellsEnd(0)) &&
             (n >= getInnerNoCommCellsBegin(1) && n < getInnerNoCommCellsEnd(1)) &&
             (o >= getInnerNoCommCellsBegin(2) && o < getInnerNoCommCellsEnd(2));
    }
    ...
    /// get start of the part of inner grid, where there is no communication (too far from boundary
    longint getInnerNoCommCellsBegin(int i) const { return 2 * frame; }
    /// get first element after inner elements
    longint getInnerNoCommCellsEnd(int i) const { return Grid::getGridSize(i) - 2 * frame; }
