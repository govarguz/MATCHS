...
namespace espressopp {
  namespace storage {
    ...
    */
    ...
    private:
      /// actual constructor of the class
      /// it is called from the two variants of constructors
      ...
      int boundaries[6];
      // max Domain size of the cells
      int maxDomainSizeInCells[3];
      // the Neighbor Lists
      std::vector<int> neiListx;
      std::vector<int> neiListy;
      std::vector<int> neiListz;

      // Get node Pos to calc the properties of a Node
      Int3D localNodePos;

      /// cell size
      /// pkus: isn't it rather physical size of part of the computational domain belonging to the given node?
      Real3D localBoxSize;
      /// inverse domain size
