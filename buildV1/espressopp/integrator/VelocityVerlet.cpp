  ...
  namespace integrator {
    ...
    void VelocityVerlet::run(int nsteps)
    {
      ...
      for (int i = 0; i < nsteps; i++) {
        ...
        if (resortFlag) {
//            std::cout << "particles have to be resorted " << maxDist << ", " << skinHalf << std::endl;
            VT_TRACER("resort1");
            time = timeIntegrate.getElapsedTime();
      ...
      real overheadUpdateGhosts, overheadCollectForces, overheadDecomposeReal, overheadExchangeGhosts;
      system.storage->getTimeOverheads(overheadUpdateGhosts, overheadCollectForces, overheadDecomposeReal, overheadExchangeGhosts);
      timeOverhead = overheadUpdateGhosts + overheadCollectForces + overheadDecomposeReal + overheadExchangeGhosts;
      timeComm1 -= overheadUpdateGhosts;
      timeComm2 -= overheadCollectForces;
      timeResort -= (overheadDecomposeReal + overheadExchangeGhosts);
    ...
    // see http://stackoverflow.com/questions/31295122/boost-python-multiple-return-arguments
    static boost::python::tuple wrapGetTimers(class VelocityVerlet* obj) {
      real tms[11];
      obj->loadTimers(tms);
    ...
    void VelocityVerlet::loadTimers(real t[11]) {
      t[0] = timeRun;
      t[1] = timeForceComp[0];
      t[2] = timeForceComp[1];
      t[3] = timeForceComp[2];
      ...
      t[8] = timeResort;
      t[9] = timeLost;
      t[10] = timeOverhead; // overhead of the duplicate approach (transfering particles between layers)
    }
    ...
    void VelocityVerlet::printTimers() {
      ...
      cout << "overhead (%) = " << timeLost << " (" << pct << ")" << endl;
      cout << endl;
    }
