...
namespace espressopp {
  namespace storage {
    ...
    public:
      ...
      /// halfCellMasks contain as many integers in each dirrections, as many nodes there are
      /// the halfCellInt of node (nx, ny, nz) is determined as
      /// min(halfCellMaskx[nx], halfCellMasky[ny], halfCellMaskz[nz])
      /// the idea is, that we will probably need half cell or finer in the middle of the domain
      /// it is also consistent with product-like creation of cell grid based on neiLists
      DomainDecomposition(shared_ptr< System > system,
              const Int3D& _nodeGrid,
              const std::vector<int>& neiListx,
              const std::vector<int>& neiListy,
      ...
      virtual Cell *mapPositionToCellWithGhosts(const Real3D& pos);  // Hcomment: Added by Pavel
    ...
    //protected:
    public: //TODO I need it to be able to use it in the derived template .. 
            //TODO I do not understand why...
      ...
      void printAllParticles(std::string where, std::vector<int> only_particles);
      void printNodeParticles(int rank);
    ...
    protected:
      ...
      /// find node neighbours and log information
    ...
    private:
      ...
      void printCellParticles(Cell* cell, std::string where,  bool isReal, std::vector<int> only_particles);
    };
  }
}
#endif
