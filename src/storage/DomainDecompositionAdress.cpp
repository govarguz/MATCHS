...
namespace espressopp { 
  ...
  DomainDecompositionAdress(shared_ptr< System > _system,
    ...
    LOG4ESPP_INFO(logger, "node grid = "
          ...
          << _cellGrid[0] << "x" << _cellGrid[1] << "x" << _cellGrid[2]);
          //<< " NeiList grid = "
          //<< neiList[0][0] << "x" << neiList[1][0] << "x" << neiList[2][0]); // it should give 0,0,0 !
  ...
  void DomainDecompositionAdress::construct(const Int3D& _cellGrid)
  {
      createCellGrid(_cellGrid);
      initCellInteractions();
      prepareGhostCommunication();
//      std::cout << "real cells " << realCells.size() << std::endl;
//      std::cout << "position " << nodeGrid.getNodePosition(0) << ", " << nodeGrid.getNodePosition(1) << ", " << nodeGrid.getNodePosition(2) << std::endl;
  }

  void DomainDecompositionAdress::
  createCellGrid(const Int3D& _cellGrid) {
    real myLeft[3];
    real myRight[3];
  ...
  void DomainDecompositionAdress::cellAdjust(){
      ...
      const boost::python::list neiListxV;
      const boost::python::list neiListyV;
      const boost::python::list neiListzV;
  ...
  */
  ...
  class PyDomainDecompositionAdress : public DomainDecompositionAdress {
      public:
        PyDomainDecompositionAdress(shared_ptr< System > _system,
                  ...
                  const boost::python::list& neiListy,
                  const boost::python::list& neiListz)
      : DomainDecompositionAdress(_system, _halfCell, _nodeGrid, _cellGrid, neiListx, neiListy, neiListz)
        {}
      };
  ...
  ("storage_DomainDecompositionAdress",
   ...
   const Int3D&, const Int3D&, const boost::python::list&, const boost::python::list&, const boost::python::list& >())
